...
function obtenerRutaDesdeURL(url?: string): string[] {
  if (!url) return []
  const base = url.split('?')[0] || ''
  const normalizada = base.replace(/^\/+|\/+$/g, '')
  const prefijo = 'api/proyectos'
  if (!normalizada.startsWith(prefijo)) return []
  const resto = normalizada.slice(prefijo.length).replace(/^\/+/, '')
  if (!resto) return []
  return resto.split('/').filter(Boolean)
}

function obtenerRuta(req: Solicitud): string[] {
  const porURL = obtenerRutaDesdeURL(req.url)
  if (porURL.length > 0) return porURL
  ...
  try {
    ...
    if (ruta.length === 1) {
      ...
      if (method === 'GET') {
        ...
        if (!proyecto) return responder(res, 404, null, 'Proyecto no encontrado')
      ...
      if (method === 'PUT') {
        ...
        if (!proyectoActualizado) return responder(res, 404, null, 'Proyecto no encontrado')
      ...
      if (method === 'DELETE') {
        ...
        if (!eliminado) return responder(res, 404, null, 'Proyecto no encontrado')
    ...
    if (
      ...
      if (!titulo) return responder(res, 400, null, 'El título de la tarea es obligatorio.')
      ...
      if (!nuevaTarea) return responder(res, 404, null, 'Columna no encontrada')
    ...
    if (
      ...
      if (method === 'PUT') {
        ...
        if (!tareaActualizada) return responder(res, 404, null, 'Tarea no encontrada')
      ...
      if (method === 'DELETE') {
        ...
        if (!eliminada) return responder(res, 404, null, 'Tarea no encontrada')
    ...
    if (method === 'PATCH' && ruta.length === 3 && ruta[0] === 'tareas' && ruta[2] === 'mover') {
      ...
      if (!tareaMovida) return responder(res, 404, null, 'Tarea o columna no encontrada')

      return responder(res, 200, tareaMovida, 'Tarea movida correctamente')
    }

    if (method === 'OPTIONS') {
      return responder(res, 200, null, 'Solicitud preflight correcta')
    }
